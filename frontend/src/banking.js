...
function Banking() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("모바일뱅킹");
  const [searchTerm, setSearchTerm] = useState("");
  const [filteredfinanceItems, setFilteredfinanceItems] = useState(financeItems);
  ...
  useEffect(() => {
    if (selectedCategory === "전체") {
      ...
      setFilteredfinanceItems(financeItems);
    } else {
      const filteredItems = searchData(searchTerm, selectedCategory);
      setFilteredfinanceItems(filteredItems);
    }
    ...
    <div class="w-full h-full">
          ...
          <div className="flex mr-8">
            ...
            >
              <span className="flex items-center">
                {selectedCategory === "true" ? "뱅킹" : selectedCategory}
              </span>
              <svg
                ...
                >
                  <li>
                    <button
                      ...
                      onClick={() => handleDropdownItemClick("모바일뱅킹")}
                    >
                     모바일뱅킹
                    </button>
                  </li>ㄴ
              ...
              <div>
                <div class="grid grid-cols-1 mt-8 md:grid-cols-5 gap-16 hidden lg:flex">
                  <button
                    type="button"
                    data-value="국민은행"
                    onClick={() => handleButtonClick("국민은행")} 
                  ...
                  <button
                    type="button"
                    data-value="신한은행"
                    onClick={() => handleButtonClick("신한은행")} 
                  ...
                  <button
                    type="button"
                    data-value="카카오뱅크"
                    onClick={() => handleButtonClick("카카오뱅크")}  // 버튼을 누를 때 검색어를 "샐러디"로 설정
                    class="text-[16px] text-gray-500 hover:text-white bg-white hover:bg-indigo-300 focus:ring-1 focus:outline-none focus:ring-indigo-300 font-bold rounded-full px-2 py-2 text-center mb-2 w-32"
                  >
                    카카오뱅크
                  </button>
      ...
      <div>
        <div className="grid gird-cols-2 md:grid-cols-4 p-20 gap-10">
          {filteredfinanceItems.map((data, index) => (
            <div key={index}>
